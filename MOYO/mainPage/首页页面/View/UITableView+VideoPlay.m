...
- (void)playVideoInVisiableCells{
    ...
    MY_TTLActiveHomeTableViewCell *videoCell = nil;
    
    for (MY_TTLActiveHomeTableViewCell *cell in visiableCells) {
    ...
    if (videoCell) {
        ...
        // display status view.
//        [videoCell.imgvHead jp_playVideoWithURL:[NSURL URLWithString:videoCell.videoPath]];
         [videoCell.imgvHead jp_playVideoWithURL:[NSURL URLWithString:videoCell.videoPath] options:JPVideoPlayerContinueInBackground | JPVideoPlayerLayerVideoGravityResizeAspectFill | JPVideoPlayerShowActivityIndicatorView | JPVideoPlayerShowProgressView progress:nil completed:nil];
    }
}
...
- (void)handleScrollStop{
    MY_TTLActiveHomeTableViewCell *bestCell = [self findTheBestToPlayVideoCell];
    ...
    if (self.playingCell.hash != bestCell.hash && bestCell.hash != 0) {
        ...
        [self.playingCell.imgvHead jp_stopPlay];
        ...
        // hide status view.
//         [bestCell.imgvHead jp_playVideoWithURL:url];
        [bestCell.imgvHead jp_playVideoWithURL:url options:JPVideoPlayerContinueInBackground | JPVideoPlayerLayerVideoGravityResizeAspectFill | JPVideoPlayerShowActivityIndicatorView | JPVideoPlayerShowProgressView progress:nil completed:nil];
...
- (MY_TTLActiveHomeTableViewCell *)findTheBestToPlayVideoCell{
    
    // To find next cell need play video.
    // 找到下一个要播放的cell(最在屏幕中心的).
    
    MY_TTLActiveHomeTableViewCell *finnalCell = nil;
    NSArray *visiableCells = [self visibleCells];
...
- (void)setPlayingCell:(MY_TTLActiveHomeTableViewCell *)playingCell{
    objc_setAssociatedObject(self, @selector(playingCell), playingCell, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (MY_TTLActiveHomeTableViewCell *)playingCell{
