...
//    [array addObjectsFromArray:localAry];
    ...
    for (int i =0; i < 60; i++) {
        ...
        NSString *nameStr = [NSString stringWithFormat:@"%03d",i+1];
        
        [array addObject:nameStr];
        
    }
     */
    NSArray *arry =  @[@"[呲牙]",@"[大笑]",@"[示爱]",@"[晕]",@"[流泪]",@"[调皮]",@"[抓狂]",
                       @"[恼]",@"[脸红]",@"[愤怒]",@"[流汗]",@"[愉快]",@"[打呼]",@"[金钱]",@"[捂嘴]",
                       @"[抽烟]",@"[衰]",@"[疑惑]",@"[惊讶]",@"[鄙视]",@"[抠鼻]",@"[色]",@"[鼓掌]",
                       @"[皱眉]",@"[思考]",@"[阴沉]",@"[喜悦]",@"[积极]",@"[纠结]",@"[右瞪]",@"[左瞪]",@"[嘘]",
                       @"[委屈]",@"[哈欠]",@"[砸]",@"[疑问]",@"[眨眼]",@"[羞羞]",@"[快哭了]",@"[挥手]",@"[尴尬]",@"[赞]",
                       @"[藐视]",@"[给心]",@"[摇手]",@"[ok]",@"[勾引]",@"[胜利]",@"[拳头]",@"[挑衅]",@"[握手]",@"[玫瑰]",
                       @"[爱心]",@"[心碎]",@"[猪]",@"[咖啡]",@"[晚安]",@"[太阳]",@"[礼物]",@"[蛋糕]"];
    return arry;
}
