...
- (instancetype)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame]) {
        ...
        [self setUp];
        [[NSNotificationCenter defaultCenter]  addObserver:self selector:@selector(setTabBarIndexStr:)  name:@"NotifyTabBarIndex" object:nil];
        self.tabBarIndex = 0;
    }
...
- (void)setUp
{
    ...
    self.backgroundColor = [Utility colorWithHexString:@"f43530" alpha:1.0];
...
- (BOOL)pointInside:(CGPoint)point withEvent:(UIEvent*)event
{
    ...
    if ([TabBarController shareInstance].selectedTabBarIndex > 0) {
        bounds = CGRectZero;
    }
