...
+ (void)initialize
{
    ...
    cell.rightBubbleMargin = UIEdgeInsetsMake(5, 10, 5, 15);
                         ...
                         model:(id<IMessageModel>)model
{
    ...
    if ([self respondsToSelector:@selector(isCustomBubbleView:)] && [self isCustomBubbleView:model]) {
        ...
        if (model.isSender) {
            switch (messageType) {
                ...
                {
                    self.leftBubbleMargin = UIEdgeInsetsMake(1, 15, 1, 1);
                    self.rightBubbleMargin = UIEdgeInsetsMake(1, 1, 1, 15);
                ...
                case EMMessageBodyTypeVideo:
                {
                    ...
                    _bubbleView.videoTagView.image = [UIImage imageNamed:@"messageVideoly"];
                }
                    break;
    ...
    [self.VipLevelLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.right.mas_equalTo(self.avatarView.mas_right).mas_offset(5);
...
//    self.bubbleMaxWidthConstraint.active = YES;
    ...
    [self addConstraint:[NSLayoutConstraint constraintWithItem:self.statusButton attribute:NSLayoutAttributeCenterY relatedBy:NSLayoutRelationEqual toItem:self.contentView attribute:NSLayoutAttributeCenterY multiplier:1.0 constant:-5]];
    ...
    [self addConstraint:[NSLayoutConstraint constraintWithItem:self.activity attribute:NSLayoutAttributeCenterY relatedBy:NSLayoutRelationEqual toItem:self.contentView attribute:NSLayoutAttributeCenterY multiplier:1.0 constant:-5.0]];
    
    [self _updateHasReadWidthConstraint];
    
    [self addConstraint:[NSLayoutConstraint constraintWithItem:self.hasRead attribute:NSLayoutAttributeHeight relatedBy:NSLayoutRelationEqual toItem:nil attribute:NSLayoutAttributeHeight multiplier:1.0 constant:15]];
...
//                if ([model.message.ext[@"realTimeCall"] isEqualToString:@"YES"]) {
//                    
            ...
            case EMMessageBodyTypeVoice:
            {
                if (_bubbleView.voiceImageView) {
                    if ([self.sendMessageVoiceAnimationImages count] > 0 && [self.recvMessageVoiceAnimationImages count] > 0) {
                        ...
                        self.bubbleView.voiceImageView.image = self.model.isSender ?[UIImage imageNamed:@"chat_sender_audio_ly_full"]: [UIImage imageNamed:@"chat_receiver_audio_playing_full"];
                    }
                }
...
//    CGFloat height = EaseMessageCellPadding + cell.bubbleMargin.top + cell.bubbleMargin.bottom;
    CGFloat height = EaseMessageCellPadding + 5;  // + cell.bubbleMargin.bottom;
    ...
    switch (model.bodyType) {
        case EMMessageBodyTypeText:
        {
            ...
            height += (rect.size.height > 15 ? rect.size.height : 15) + 10;
