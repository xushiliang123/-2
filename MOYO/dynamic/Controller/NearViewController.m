...
@interface NearViewController ()<ZFPlayerDelegate,VideoTableViewCellDelegate,MY_LLPayViewControllerDelegate,GetifpayattentionDelegate,MY_LLChatRedPacketViewDelegate,MY_LLChatRedPacketSayHWViewDelegate>
{
//    UITableView * _tabbliView;
    NSInteger _page;
    VideoTableViewCell *_currRedcell; // 当前点击的cell发送的小红包
    VideoTableViewCell *_currattentioncell; // 当前点击的cell上的关注
    VideoTableViewCell *_playCell;//当前播放视屏的cell
    MY_LYPersonInfoModel *_personalModel;
...
-(void)initUI
{
    ...
    NSMutableAttributedString *attributeStr = [[NSMutableAttributedString alloc] initWithString:@"附近还没有人发动态呢\n不如自己先发个动态吧！" attributes:dic];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    cell.personalModel = _personalModel; //把我自己的model传进去
    cell.delegate = self;
    // 赋值model
    cell.model = model;
...
- (void)avatarBtnClick:(UIButton *)btn {
    ...
    _currattentioncell = [self.tableView cellForRowAtIndexPath:indexPath];
...
/*
 ...
 @property (strong, nonatomic) NSNumber* ;      //图片的类型，0为图片，1为视频截图图片
 */
...
//        [chatController setHidesBottomBarWhenPushed:YES];
        ...
        float enco = _currRedcell.model.encourageMoney.floatValue;
        float addP = price.floatValue;
        int currMoney =  (int)(enco + addP);
        _currRedcell.hongBaoChat.title = [NSString stringWithFormat:@"%d",currMoney];
        _currRedcell.model.encourageMoney = @(currMoney);
    }
...
-(void)GetifpayattentionWithStatus:(BOOL)payattention
{
    if(payattention)
    {
        _currattentioncell.attentionBtn.hidden = YES;
    }
    else
    {
        _currattentioncell.attentionBtn.hidden = NO;
    }
...
- (void)viewWillDisappear:(BOOL)animated {
    ...
    [_playCell.effectView removeFromSuperview];
}
