//
//  MY_XTAddManagerViewController.m
//  VTIME
//
//  Created by tingting on 2017/10/19.
...
@interface MY_XTAddManagerViewController ()<UISearchBarDelegate>
{
    NSString* _strSearch;
    NSInteger _page;
}
...
@implementation MY_XTAddManagerViewController
...
- (void)rightBarAction:(UIButton *)btn {
    ...
    [TLHTTPRequest MY_postWithBaseUrl:ClubSetManagerUrl parameters:@{@"clubId":self.clubId,@"inviteList":strList} success:^(NSURLSessionDataTask * _Nonnull task, NSDictionary * _Nonnull data) {
        [LQProgressHud hide];
        if ([data[@"ret"] intValue] == 0) {
//            NSLog(@"ClubInviteFriendJoin-- :%@",data);
...
- (void)downLoadData{
    ...
    [self.dataArray addObjectsFromArray:self.dataA];
    if (self.dataArray.count>0)
    {
        [self.tableView reloadData];
    }
