...
//  XCIntegrateApp
//
//  Created by 羊羊 on 16/8/10.
//  Copyright © 2016年 崔玉国. All rights reserved.
//

#import "XCRatingBar.h"

@interface XCRatingBar ()
{
    float starRating;
    float lastRating;
...
//    fullSelectedImage = [UIImage imageNamed:fullSelectedName];
    ...
    if (height < [fullSelectedImage size].height) {
        height = [fullSelectedImage size].height-15;
    }
    if (height < [halfSelectedImage size].height) {
        height = [halfSelectedImage size].height-15;
    }
    if (height < [unSelectedImage size].height) {
        height = [unSelectedImage size].height-15;
    }
    if (width < [fullSelectedImage size].width) {
        width = [fullSelectedImage size].width-15;
    }
    if (width < [halfSelectedImage size].width) {
        width = [halfSelectedImage size].width-15;
    }
    if (width < [unSelectedImage size].width) {
        width = [unSelectedImage size].width-15;
    }
...
-(void)displayRating:(float)rating{
    ...
    starRating = rating;
    lastRating = rating;
    [_delegate ratingChanged:rating viewTag:self.tag];
}
