...
@interface AttentionViewController () <ZFPlayerDelegate,VideoTableViewCellDelegate,MY_LLPayViewControllerDelegate,GetifpayattentionDelegate,MY_LLChatRedPacketViewDelegate,MY_LLChatRedPacketSayHWViewDelegate>
{
    NSInteger _page;
...
- (instancetype)init
{
    ...
    if (self) {
        ...
        [defaultCenter addObserver:self selector:@selector(xq_headerRefreshing) name:@"FindRefreshData" object:nil];
    }
    return self;
}
...
//    [self.tableView registerNib:[UINib nibWithNibName:@"FindTableViewCell" bundle:nil] forCellReuseIdentifier:@"FindTableViewCell"];
    self.tableView.backgroundColor = [Utility colorWithHexString:@"e5e5e5" alpha:1.0];
    ...
    self.nullTitleLabel.text = @"你还没有关注其他人哦~";
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (model.dynamicType.integerValue) {
        ...
        cell.playBlock = ^(UIButton *btn){
            ...
            [self xq_addViewLargeImageWithModel:model cell:weakCell];
    ...
    cell.ViewLargeImageBlock = ^(VideoResolutionModel *model, CGFloat cellOriginY) {
        ...
        [weakSelf xq_viewLargeImageWithModel:model cell:weakCell];
    };
    
    if (cell.avatarBtn) {
        cell.avatarBtn.tag = 300 + indexPath.section;
        [cell.avatarBtn addTarget:self action:@selector(xq_avatarBtnClick:) forControlEvents:UIControlEventTouchUpInside];
    }
    else {
        [cell.avatarBtn removeTarget:self action:@selector(xq_avatarBtnClick:) forControlEvents:UIControlEventTouchUpInside];
    }
    
    
    if (cell.attentionBtn) {
        cell.attentionBtn.tag = 500 + indexPath.section;
        [cell.attentionBtn addTarget:self action:@selector(xq_attentionBtnClick:) forControlEvents:UIControlEventTouchUpInside];
    }
    else {
        [cell.attentionBtn removeTarget:self action:@selector(xq_attentionBtnClick:) forControlEvents:UIControlEventTouchUpInside];
    }
    
    
    if (cell.zanBtn) {
        cell.zanBtn.tag = 1000 + indexPath.section;
        [cell.zanBtn addTarget:self action:@selector(xq_zanBtnClick:) forControlEvents:UIControlEventTouchUpInside];
    }
    else {
        [cell.zanBtn removeTarget:self action:@selector(xq_zanBtnClick:) forControlEvents:UIControlEventTouchUpInside];
    }
    
    if (cell.privateChat) {
        cell.privateChat.tag = 2000 + indexPath.section;
        [cell.privateChat addTarget:self action:@selector(xq_privateChatBtnClick:) forControlEvents:UIControlEventTouchUpInside];
    }
    else {
        [cell.privateChat removeTarget:self action:@selector(xq_privateChatBtnClick:) forControlEvents:UIControlEventTouchUpInside];
    }
    
    UILongPressGestureRecognizer *longPressed = [[UILongPressGestureRecognizer alloc]initWithTarget:self action:@selector(xq_longPressedAct:)];
...
//  长按举报
-(void)xq_longPressedAct:(UILongPressGestureRecognizer *)gesture{
...
//            [reportVC setHidesBottomBarWhenPushed:YES];
        ...
        UIAlertController * alertCtr = [UIAlertController alertControllerWithTitle:@"举报该动态" message:@"" preferredStyle:UIAlertControllerStyleActionSheet];
...
//  头像点击 tag  300 +
- (void)xq_avatarBtnClick:(UIButton *)btn {
...
//  关注
- (void)xq_attentionBtnClick:(UIButton *)btn {
...
//  点赞  tag 1000 +
- (void)xq_zanBtnClick:(UIButton *)button {
...
//  私聊
- (void)xq_privateChatBtnClick:(UIButton *)btn {
    ...
    if (model.userId.integerValue == [USERID integerValue]) {
        ...
        [self xq_deleteDyanmicWithModel:model];
    }
    else {
        
        [self xq_ChatStatus:model];
    }
}

- (void)xq_ChatStatus:(VideoResolutionModel *)model {
    ...
    [TLHTTPRequest MY_getWithBaseUrl:ChatStatusUrl parameters:dict success:^(NSURLSessionDataTask * _Nonnull task, NSDictionary * _Nonnull data) {
        ...
        if ([[data objectForKey:@"ret"] boolValue] == 0) {
            ...
            if ([infoDict[@"status"] integerValue] == 1) {
                [self xq_pushChatVCWithModel:model];
            }
            else {
                [self xq_helloWithModel:model];
            }
        }
    } failure:^(NSURLSessionDataTask * _Nonnull task, NSError * _Nonnull error) {
        [LQProgressHud showMessage:@"请求失败，请检查网络"];
...
//  跳转聊天
- (void)xq_pushChatVCWithModel:(VideoResolutionModel *)userModel {
...
//  删除自己动态
- (void)xq_deleteDyanmicWithModel:(VideoResolutionModel *)model {
...
//  打招呼红包
- (void)xq_helloWithModel:(VideoResolutionModel *)model {
    _noteBt = 1;
    __weak typeof(self) wSelf = self;
...
#pragma mark ----- 浏览量 -----
- (void)xq_addViewLargeImageWithModel:(VideoResolutionModel *)model cell:(VideoTableViewCell *)cell{
    NSMutableDictionary* dict = [[NSMutableDictionary alloc]init];
...
- (void)xq_viewLargeImageWithModel:(VideoResolutionModel *)model cell:(VideoTableViewCell *)cell {
    
    [self xq_addViewLargeImageWithModel:model cell:cell];
