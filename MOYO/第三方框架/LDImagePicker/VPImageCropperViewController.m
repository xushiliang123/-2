...
- (void)initControlBtn {
    ...
    UIView *backv = [[UIView alloc]initWithFrame:CGRectMake(0, self.view.frame.size.height - 50.0f, SCREEN_WIDTH, 50)];
...
-(UIImage *)getSubImage{
    ...
    CGFloat scaleRatio = self.cropFrame.size.width / self.originalImage.size.width;
    CGFloat x = (squareFrame.origin.x - self.cropFrame.origin.x) / scaleRatio;
    CGFloat y = (squareFrame.origin.y - self.cropFrame.origin.y) / scaleRatio;
    CGFloat w = squareFrame.size.width / scaleRatio;
    CGFloat h = squareFrame.size.height / scaleRatio;
    if (self.cropFrame.size.width < self.cropFrame.size.width) {
        CGFloat newW = self.originalImage.size.width;
        CGFloat newH = newW * (self.cropFrame.size.height / self.cropFrame.size.width);
        x = 0; y = y + (h - newH) / 2;
        w = newH; h = newH;
    }
    if (self.cropFrame.size.height < self.cropFrame.size.height) {
