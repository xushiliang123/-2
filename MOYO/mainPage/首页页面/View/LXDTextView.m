...
- (void)commonInit
{
    ...
    [CoverBtn addTarget:self action:@selector(bottomLblViewAction:) forControlEvents:UIControlEventTouchUpInside];
...
//            NSLog(@"%lu ---- %lu, %@ --- %@", subLocation, range.location, hyperlinkText, subText);
            ...
            [_content addAttributes: @{ NSForegroundColorAttributeName: [Utility colorWithHexString:@"d9b878" alpha:1.0] } range: NSMakeRange(range.location + subLocation, range.length)];
            [_content addAttributes: @{ NSUnderlineStyleAttributeName: @(NSUnderlineStyleNone) } range: range];
    ...
    CTFontRef font = CTFontCreateWithName((CFStringRef)[UIFont systemFontOfSize: 22].fontName, 14, NULL);
...
- (void)drawRect: (CGRect)rect
{
    ...
    CGRect textRect = CGRectInset(self.bounds, 5, 10);
...
- (void)bottomLblViewAction:(UIButton*)sender{
    if ([_delegate respondsToSelector: @selector(textView:didSelectedHyperlink:)]) {
        [_delegate textView: self didSelectedHyperlink: nil];
    }
}
