...
@interface MY_XTBundWXViewController ()<UITableViewDelegate,UITableViewDataSource>
{
    UITextField *WXnum;
    UITextField *WXnumagain;
    UITextField *WXname;
...
@implementation MY_XTBundWXViewController
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.row == 0)
    {
        ...
        CGSize s = [Utility stringSizeWithString:@"微信账号" textSize:16 width:0 height:15];
        
        WXnum = [[UITextField alloc]initWithFrame:CGRectMake(16 + s.width + 14, 17.5, 210 * SCREEN_WIDTH / 375, 15)];
        
        WXnum.placeholder = @"请输入微信号（必填）";
        WXnum.text = self.number;
        WXnum.font = [UIFont systemFontOfSize:16];
        WXnum.textColor = [Utility colorWithHexString:@"#3C3C3C" alpha:1.0f];
        [cell.contentView addSubview:WXnum];
    ...
    else if(indexPath.row == 1)
    {
        ...
        UILabel *textLabel1 = [[UILabel alloc]initWithFrame:CGRectMake(16, 17.5, 120, 15)];
        textLabel1.text = @"确认微信账号";
        textLabel1.font = [UIFont systemFontOfSize:16];
        textLabel1.textColor = [Utility colorWithHexString:@"#3C3C3C" alpha:1.0f];
        textLabel1.textAlignment = NSTextAlignmentLeft;
        [cell.contentView addSubview:textLabel1];
        
        CGSize s = [Utility stringSizeWithString:@"确认微信账号" textSize:16 width:0 height:15];
        
        WXnumagain = [[UITextField alloc]initWithFrame:CGRectMake(16 + s.width + 14, 17.5, 210 * SCREEN_WIDTH / 375, 15)];
        
        WXnumagain.placeholder = @"再次输入微信号（必填）";
        WXnumagain.text = self.number;
        WXnumagain.font = [UIFont systemFontOfSize:16];
        WXnumagain.textColor = [Utility colorWithHexString:@"#3C3C3C" alpha:1.0f];
        [cell.contentView addSubview:WXnumagain];
    ...
    else if(indexPath.row == 2)
    {
        ...
        UILabel *textLabel1 = [[UILabel alloc]initWithFrame:CGRectMake(16, 17.5, 120, 15)];
        textLabel1.text = @"请输入微信实名";
        textLabel1.font = [UIFont systemFontOfSize:16];
        textLabel1.textColor = [Utility colorWithHexString:@"#3C3C3C" alpha:1.0f];
        textLabel1.textAlignment = NSTextAlignmentLeft;
        [cell.contentView addSubview:textLabel1];
        
        CGSize s = [Utility stringSizeWithString:@"请输入微信实名" textSize:16 width:0 height:15];
        
        WXname = [[UITextField alloc]initWithFrame:CGRectMake(16 + s.width + 14, 17.5, 210 * SCREEN_WIDTH / 375, 15)];
        
        WXname.placeholder = @"请输入微信实名（必填）";
        WXname.text = self.name;
        WXname.font = [UIFont systemFontOfSize:16];
        WXname.textColor = [Utility colorWithHexString:@"#3C3C3C" alpha:1.0f];
        [cell.contentView addSubview:WXname];
...
-(void)BundButtonClick:(UIButton *)btn
{
    ...
    if (![Utility isBlankString:WXnum.text] && ![Utility isBlankString:WXnumagain.text] ) {
        if([WXnum.text isEqualToString:WXnumagain.text])
        {
            [dict setObject:WXnum.text forKey:@"vxNo"];
        }
        else{
            [LQProgressHud showMessage:@"两次输入的微信号不同"];
        }
    }
    else{
        [LQProgressHud showMessage:@"请输入微信号"];
        return;
    }
    
    if (![Utility isBlankString:WXname.text]) {
        [dict setObject:WXname.text forKey:@"vxName"];
    }
    else{
