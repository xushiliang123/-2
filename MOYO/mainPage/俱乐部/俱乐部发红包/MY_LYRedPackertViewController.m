...
#import "MY_LYRedPackertViewController.h"
#import "RechargeViewController.h"
@interface MY_LYRedPackertViewController ()<UITextViewDelegate,UITextFieldDelegate>
{
...
@implementation MY_LYRedPackertViewController
...
-(void)drawUI{
    ...
    [self addNavBtn:@"取消" isLeft:YES target:self action:@selector(clickBackButton:) bgImageName:nil];
...
//    CGFloat width = SCREEN_WIDTH - 2*(_backButton.frame.origin.x + _backButton.frame.size.width);
//    CGRect frame = CGRectMake((SCREEN_WIDTH - width)/2, 28, width, 24);
...
//    [self.view addSubview:_titleLabel];
    ...
    // 警告
    self.alertLab = [[UILabel alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH , 36)];
    ...
    [self.view addSubview:redMoneyView];
    ...
    if (self.clubId.intValue != 0) {
        self.moneyTextField.text = @"200";
    }else{
        self.moneyTextField.text = @"100";
    }
    ...
    [self.view addSubview:redCountView];
    ...
    int hongBaoCount = 10;
    if (self.clubId.intValue != 0) {
        hongBaoCount = self.clubPersonalNum.intValue;
    }
    self.inputTextField.text = [NSString stringWithFormat:@"%d",hongBaoCount];
    ...
    [self.inputTextField addTarget:self action:@selector(moneyTextFieldTextChange:) forControlEvents:UIControlEventEditingChanged];
    ...
    //  人数
//    UILabel * bestHighLab = [[UILabel alloc]initWithFrame:CGRectMake(14, CGRectGetMaxY(redCountView.frame), SCREEN_WIDTH , 26)];
...
//    [self.view addSubview:bestHighLab];
    
    self.desTextView = [[UITextView alloc]initWithFrame:CGRectMake(14, CGRectGetMaxY(redCountView.frame) + 22, SCREEN_WIDTH - 30, 64)];
    ...
    // 金额
    self.redPacketMoneyLable = [[UILabel alloc]initWithFrame:CGRectMake(0, CGRectGetMaxY(self.desTextView.frame) + 40, SCREEN_WIDTH, 42)];
    ...
    self.redPacketMoneyLable.text = money;
    [self.view addSubview:self.redPacketMoneyLable];
    
    self.moneyLogoLable = [[UILabel alloc]initWithFrame:CGRectMake(SCREEN_WIDTH/2-20, CGRectGetMaxY(self.desTextView.frame) + 50, 21, 22)];
    CGSize size = [Utility stringSizeWithString:self.redPacketMoneyLable.text textSize:40 width:0 height:SCREEN_WIDTH];
    self.moneyLogoLable.mj_x = (SCREEN_WIDTH - size.width)/2-20;
    ...
    [self.view addSubview:self.moneyLogoLable];
    ...
    [self.view addSubview:addStatus];
...
-(void)sendRedPacketButtonClickMethod:(UIButton*)sender{
    ...
    NSString * money= nil;
    NSInteger limtCount = 5;
    money = @"200";
    //  如果房间ID = 0 则是世界频道
    if (![self.clubId integerValue]) {
        limtCount = 10;
        money = @"100";
    }
    
    if ([self.inputTextField.text integerValue] < limtCount || [self.inputTextField.text integerValue] > [self.clubPersonalNum integerValue]) {
        //  红包个数限制 1 - clubPersonalNum
        
        titleStr =  [NSString stringWithFormat:@"塞红包要%td- %td 个哦！",limtCount,[self.clubPersonalNum integerValue]];
        [LQProgressHud showMessage:titleStr];
        return;
    }
    
    // 红包金额
    if ([self.moneyTextField.text integerValue] < [money integerValue] || [self.moneyTextField.text integerValue] > 99999999) {
        //  红包金额限制 500 - 99999999

        titleStr =  [NSString stringWithFormat:@"塞红包要%@- 9999999元哦！",money];
    ...
    [TLHTTPRequest MY_postWithBaseUrl:SendMultipleHongbao parameters:diction success:^(NSURLSessionDataTask * _Nonnull task, NSDictionary * _Nonnull data) {
        if ([data[@"ret"] integerValue] == 0) {
            ...
            if ([diction[@"payMode"] integerValue] == 0) {
                ...
                // 红包 余额 支付成功
                if (weakSelf.backClickMethod) {
                    
                    weakSelf.backClickMethod(weakSelf ,YES);
                }
            }
...
//    NSString * titleStr = nil;
    ...
    if (textField == self.moneyTextField) {
        ...
        if ([self.moneyTextField.text integerValue] < [money integerValue] ) {
            
            self.alertLab.hidden = NO;
            self.alertLab.text =  [NSString stringWithFormat:@"红包金额不可小于%@元，个数不可小于%td",money,limtCount];
        }else{
            self.alertLab.hidden = YES;
        }
...
- (void)moneyTextFieldTextChange:(UITextField*)txf{
    ...
    if (txf == self.moneyTextField) {
        ...
        CGSize size = [Utility stringSizeWithString:txf.text textSize:40 width:0 height:SCREEN_WIDTH];
        self.moneyLogoLable.mj_x = (SCREEN_WIDTH - size.width)/2-20;
        
        
        
    }else if (txf == self.inputTextField){
