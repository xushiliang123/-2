...
//    self.frame.size.height
    _inputTextView = [[EaseTextView alloc] initWithFrame:CGRectMake(self.horizontalPadding, self.verticalPadding, self.frame.size.width - self.verticalPadding * 2, 46 - self.verticalPadding * 2)];
...
{
    if (_faceView == nil) {
        _faceView = [[EaseFaceView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(_toolbarView.frame), self.frame.size.width, 217)];
...
//    self.toolbarView.frame.size.height
    ...
    CGFloat itemHeight = _inputTextView.frame.size.height;
    ...
    for (id item in inputViewLeftItems) {
        if ([item isKindOfClass:[EaseChatToolbarItem class]]) {
            ...
            if (chatItem.button) {
                ...
                itemFrame.origin.y = (46 - itemFrame.size.height) / 2;
...
- (void)setInputViewRightItems:(NSArray *)inputViewRightItems
{
    ...
    CGFloat itemHeight = self.inputTextView.frame.size.height;
//    CGFloat itemHeight = self.toolbarView.frame.size.height - self.verticalPadding * 2;
...
- (void)setBackgroundImage:(UIImage *)backgroundImage{
    ...
    self.toolbarBackgroundImageView.image = backgroundImage;
}
...
- (void)_willShowBottomHeight:(CGFloat)bottomHeight
{
    ...
    CGFloat toHeight = toolbarFrame.size.height + bottomHeight;
    
    if (fromFrame.size.height - toHeight < 0) {
...
//            rect.origin.y = CGRectGetMaxY(self.toolbarView.frame);
            rect.origin.y = CGRectGetMaxY(_inputTextView.frame) + self.verticalPadding;
...
- (void)_willShowKeyboardFromFrame:(CGRect)beginFrame toFrame:(CGRect)toFrame
{
    ...
    {
        [self _willShowBottomHeight:0.0];
    }
    else{
        [self _willShowBottomHeight:toFrame.size.height];
    }
...
//                     NSString *str = [NSString stringWithFormat:@"ee_%@",attachment.imageName];
                     [attStr replaceCharactersInRange:range withString:attachment.imageName];
                 }
             }];
